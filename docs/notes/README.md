...
| [20_reference_data_open_questions.md](20_reference_data_open_questions.md) | **The reference-data build's open questions.** Forty-nine items from the eight research lanes, grouped by what has to change to answer them: the spec, the carrier set, the heat bands, the units, the register. Nine are settled — the process-coupling rule and the `site_services` split (2026-09-15); the optional load-shape magnitudes, the `(carb3_activity, process_id)` key on §3.13, the `role` column in §3.6's key and the `unit_class = storage` reading of V20 (d) (2026-09-16); and the capture train's host table replacing `abates_unit_id` and the gross-not-net emission factor (2026-09-17); the rest are open |
