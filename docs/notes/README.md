...
| [20_reference_data_open_questions.md](20_reference_data_open_questions.md) | **The reference-data build's open questions.** Forty-five items from the eight research lanes, grouped by what has to change to answer them: the spec, the carrier set, the heat bands, the units, the register. Four are settled — the process-coupling rule and the `site_services` split (2026-09-15), the optional load-shape magnitudes and the `(carb3_activity, process_id)` key on §3.13 (2026-09-16); the rest are open |
