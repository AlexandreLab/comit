...
| [17_mvp_what_it_does.md](17_mvp_what_it_does.md) | **What the MVP does.** The first build compared with COMIT today on twelve dimensions, what "parity" means as a two-hop test (V1, V1b) and why the R model runs once more as the oracle, a food-and-drink premise and a cement works walked through both models, and what the MVP gives up and defers |
| [18_mvp_feature_prioritisation.md](18_mvp_feature_prioritisation.md) | **MVP feature prioritisation.** Seventy features sorted Must / Should / Could / Won't with spec anchor, dependency and verifying test per row; milestones M0–M6 with entry, scope, exit gate and verification, M4 being the MVP exit; open calls and the spec inconsistencies met while writing |
