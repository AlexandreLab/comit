...
| [17_mvp_what_it_does.md](17_mvp_what_it_does.md) | **What the MVP does.** The first build compared with COMIT today on twelve dimensions, what "parity" means as one comparison against a coupled-off R run that is a comparison point and not ground truth, the oracle-free checks that stand beside it, a food-and-drink premise and a cement works walked through both models, and what the MVP gives up and defers |
| [18_mvp_feature_prioritisation.md](18_mvp_feature_prioritisation.md) | **MVP feature prioritisation.** Eighty-one features sorted Must / Should / Could / Won't (54 / 15 / 10 / 2) with spec anchor, dependency and verifying test per row; milestones M0–M6 with entry, scope, exit gate and verification, M4 being the MVP exit — its collapse gate now stated cross-sector under `D13`; the nine review decisions of 2026-09-08; open calls and the spec inconsistencies met while writing |
| [19_worked_example_data_flow.md](19_worked_example_data_flow.md) | **The worked examples as diagrams.** Seven Mermaid diagrams carrying the cement and food-and-drink examples' own numbers: the shared entity-to-algorithm skeleton; per example, a map of every data table with the values it holds at that premise, the base-year C8 (carrier balance) as a flow with quantities, and the 2025–2050 pathway period by period; then the field-by-field table of where the pair lands on opposite branches — mass against energy denominators, `carrier_bounded` against `activity_default` mix, §7.6 running against skipped, 728.44 → 84.88 kt against 16.83 → 0 kt. Hand-written, no generator, no `--check` — the worked examples are the source |
| [20_reference_data_open_questions.md](20_reference_data_open_questions.md) | **The reference-data build's open questions.** Sixty-one items: forty-nine from the eight research lanes, grouped by what has to change to answer them (the spec, the carrier set, the heat bands, the units, the register), ten raised by the model build and two raised reading the specification. Thirteen are settled — the process-coupling rule and the `site_services` split (2026-09-15); the optional load-shape magnitudes, the `(carb3_activity, process_id)` key on §3.13, the `role` column in §3.6's key and the `unit_class = storage` reading of V20 (d) (2026-09-16); the capture train's host table replacing `abates_unit_id` and the gross-not-net emission factor (2026-09-17); where a mass duty comes from, the CO₂ export route and the kiln process-CO₂ basis (2026-09-20); and graded cooling with a reversed cascade (2026-09-24). Item 60 on the duty-family carriers is partly settled, its data side planned in note 22; the rest are open |
| [21_mvp_slice_implementation_plan.md](21_mvp_slice_implementation_plan.md) | **The pre-M2 demonstration slice — implementation plan.** The first Python build: three synthetic premises on the unmodified reference tables, solving §5 with four of its eight objective terms and seven of its twelve constraints. Maps the slice onto note 18's `MF-` features and shows where it sits below M2's gate. Reviewed 2026-09-19, sixteen findings, two of them P0 that made the first draft unsolvable: dropping the disposal variable strands 59 `reject` rows into `heat_lt60` and 34 `co2_process` producers against two consumers, and C4 cannot bind because an incumbent's capex is sunk, so carbon alone drives the switch and it drives it at the first buildable period. Also: 38 units a cost-minimiser would build free, 10 of 15 importable carriers with no price, three live `unit_eligibility` columns unread, and a 4-year first period against §5.1's uniform Δ. Carries the admission screen, the 25-path test set and eleven implementation tasks |
| [22_duty_family_gap_plan.md](22_duty_family_gap_plan.md) | **Duty families and processes — the plan to fill the gaps.** The data side of note 20 items 60 and 61: banding the 17 `REF` duties into the three cooling bands, the cooling carriers and per-band units (and the `chiller_electric_hfo` COP 0.9 anomaly), a service for each of the 11 `OTH` rows on `electricity` (recommending a new `electric_service` carrier, a decision for Alexandre), the two `PHEAT` duties no unit reaches, splitting `EN` out of the validator's duty families, and the V34 (duties are services at a grade) checks for `make data-check`. Measured on the reference data: 89 of 427 duty rows have no unit able to serve them, routed to the note 20 items that own them. Nine tasks in six lanes, each with goal, evidence rule and verification. A plan; no data changed |
...
| [2026-08-28-carb3-site-energy-system-implementation.md](../specs/2026-08-28-carb3-site-energy-system-implementation.md) | **The specification itself — partial.** §1 scope, conventions and the fifteen design decisions `D1`–`D15`; §2 system overview; §3 data model (24 entities); §4 algorithms A1–A9 with the carrier-mix rule and the relaxation ladder; §5 the optimisation model (C1–C12) with the carrier-disposal variable; §7 emissions as a readout of the balance, with the §7.7 allocation layer; §9 performance and the four scale gates; §10 validation (V1–V31, the carrier-equivalent configuration, and the failure-mode table). §13 points at the two worked examples below. §6, §8, §11 and §12 are not yet written, and §5.3.1 and §5.6 are cited but unwritten (delivery T23) |
...
| [diagrams/](../specs/diagrams/README.md) | **Generated — do not hand-edit.** The 24 entities of §3 as an ER diagram, the same graph boxed into eight subjects, a locator table naming every non-key column, and the full attribute model |
...
| [2026-08-28-carb3-site-energy-system-worked-example-cement.md](../specs/2026-08-28-carb3-site-energy-system-worked-example-cement.md) | **Worked example — the parity case.** A cement works through `A1`–`A9` in thirteen sections: the mass denominator `D5`; a co-firing kiln as three per-fuel units under `D13`; tier-1 vintage and the stranding charge `D11`; the three emission carriers of `D15`, with the waste fuel's biogenic half becoming a credit the moment capture arrives; disposal variables making vented CO₂ a reported quantity; a CCS train inheriting its host's life; the siting cap on PV; the connection C11 breach capture causes; §7.6 reconciling to 1.96%; and the carrier-equivalent configuration `V1b` compares (T10) |
| [2026-08-28-carb3-site-energy-system-worked-example-food-drink.md](../specs/2026-08-28-carb3-site-energy-system-worked-example-food-drink.md) | **Worked example — the mechanism case, and the MVP exit.** A dairy through `A1`–`A9` on the same thirteen sections: 84 low-temperature-heat rows across eleven sectors collapsing to eight units under `D13`; a 120 °C duty with boiler, CHP, heat pump and electric resistance competing under C10; the heat pump refused at 200 °C; an existing CHP made visible by `activity_default_unit` and priced at 282 gCO₂e/kWh through §7.7's allocation layer; a dryer reject-heat leg feeding a heat pump; PV bounded by area while the CHP is not; and an export limit that sheds CHP rather than curtailing PV (T16) |
...
| Doc | Topic |
|-----|-------|
| [2026-09-25-carb3-site-sankey-report.md](../superpowers/plans/2026-09-25-carb3-site-sankey-report.md) | **A Sankey report per premise.** One offline HTML file beside each solved premise's parquet: a Sankey of the site (imports, units, and the four D16 exits) with a year slider, and four small charts of the site's evolution. Adds a `unit_flow` ledger table, since no current output says how much of which carrier each unit drew. Five tasks, all built: `carb3/src/carb3/report/`, run by `make carb3-run OUT_DIR=…` |
