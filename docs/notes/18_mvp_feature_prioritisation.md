...
| MF-03 | Unit collapse **across sectors, keyed per fuel (D13)**: 340 fuel-variant technologies and 57 non-fuel ones resolve to units keyed `(duty family or chemistry node) × primary carrier`, so the eleven sectors' gas boilers become one `boiler_lt_gas`; `grade_in`/`grade_out` on every unit and heat duty; a **lineage table** giving every one of the 397 source rows exactly one disposition (resolved to unit X, preserved as unit Z, or dropped with reason) | Must | The unit set the LP chooses from, and the mapping V1b compares through | §3.5, §3.6, §1.6 D13; migration B1–B3 | MF-01, MF-02 | `make data-check`; lineage table sums to 397 with zero unassigned rows; declared unit count matches; **exactly one `fuel_input` row per unit (V27)**; `grade_rank` non-null wherever gradeable; V19 |
...
| MF-12 | Reject-heat coefficients on every heat-consuming unit | Should | Waste heat recovery is a headline capability but the LP runs without it | §3.6 `role = reject`; migration B5 | MF-03 | V22 (c) |
...
- **Exit gate:** `make data-check` green; the lineage table assigns all 397 source rows
  ...
  `fuel_input` row per unit (V27)**; **every `(activity, set, vector)` energy share sums to
