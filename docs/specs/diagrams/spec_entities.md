...
erDiagram
    ...
    premise_record ||--|{ premise_process_energy : "premise_id"
...
flowchart LR
    ...
    subgraph d3["Duties"]
        ...
        activity_process_energy_share["activity_process_energy_share<br/>§3.3.1"]
    end
    ...
    subgraph d5["Units"]
        ...
        unit_abatement_host["unit_abatement_host<br/>§3.5.3"]
    ...
    unit_abatement_host -->|unit_id, host_unit_id| unit
    ...
    premise_process_energy -->|premise_id| premise_record
    premise_process_energy -->|process_id| activity_process_register
    premise_process_energy -->|carrier_id| carrier
    premise_process_unit -->|premise_id| premise_process_detail
    premise_process_unit -->|unit_id| unit
...
|  | `unit_abatement_host` | §3.5.3 | — | `provenance` |
...
erDiagram
    ...
    activity_process_energy_share {
    string carb3_activity PK,FK "required"
    string process_set_id PK,FK "required"
    string process_id PK,FK "required"
    enum vector PK "required"
    real energy_share "required"
    real share_low "optional"
    real share_high "optional"
    enum evidence_tier "required"
    string provenance "required"
    ...
    real biogenic_fraction "optional"
    enum carbon_charge "optional"
    enum denominator_kind "required"
    boolean may_dispose "required"
    boolean may_import "required"
    boolean may_export "required"
    }
    unit {
    ...
    boolean draws_ambient "required"
    ...
    unit_abatement_host {
    string unit_id PK,FK "required"
    string host_unit_id PK,FK "required"
    enum provenance "required"
    }
    unit_input_output {
    string unit_id PK,FK "required"
